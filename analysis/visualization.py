...
def plot_arc(positions, atom_type, dataset_info, save_path):
    w, h = dataset_info['grid_width'], dataset_info['grid_width']
    n_pixels = w * h
    assert positions.shape[0] == 2 * n_pixels, "Support only similar shapes"
    pocket = positions[:n_pixels]
    lig = positions[n_pixels:]
    ...
    axs[0].set_title("x")
    axs[1].set_title("y")
    axs[0].scatter(pocket[:, 0], pocket[:, 1], c=atom_type[:n_pixels], cmap=cmap, norm=norm)
    axs[1].scatter(lig[:, 0], lig[:, 1], c=atom_type[n_pixels:], cmap=cmap, norm=norm)
...
def visualize2d(path, dataset_info, max_num=25):
    ...
    for file in files:
        ...
        wandb.log({short_name: [wandb.Image(im, caption=short_name)]})
