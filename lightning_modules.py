...
from equivariant_diffusion.dynamics import ARCEGNNDynamics, EGNNDynamics
...
class ARCLigandPocketDDPM(pl.LightningModule):
    def __init__(
            self,
            outdir,
            dataset,
    ...
    def log_metrics(self, metrics_dict, split, batch_size=None, **kwargs):
        for m, value in metrics_dict.items():
            if m == 'loss':
                self.log(f'{m}/{split}', value, batch_size=batch_size, prog_bar = True, **kwargs)
...
#         for m, value in metrics_dict.items():
#             prog = m == 'loss'
#             self.log(f'{m}/{split}', value, batch_size=batch_size, prog_bar = prog, **kwargs)
...
#         save_coords_file(str(outdir), one_hot_flat, x_flat, self.dataset_info,
